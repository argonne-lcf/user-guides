 ...
 *
 * Add support for checkpoint/restart using Independent I/O implementation:
 * - every rank writes its data to the file independently (without coordination)
 *  - rank 0 writes additional header information
 * - on restart every rank reads its data from the file independently
 */
