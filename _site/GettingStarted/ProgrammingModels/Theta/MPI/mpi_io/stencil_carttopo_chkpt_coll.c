 ...
 *
 * Add support for checkpoint/restart using Collective I/O implementation:
 * - every rank writes its data to the file collectively (with coordination)
 *  - rank 0 writes additional header information
 * - on restart every rank reads its data from the file collectively
 */
...
int write_checkpoint_coll(char *prefix, int procs, int n, int *coords, int bx, int by, int iter,
                          double *buf);

int read_checkpoint_coll(char *prefix, int procs, int n, int *coords, int bx, int by, int iter,
                         double *buf);
                          ...
                          double *buf)
{
    ...
    err = MPI_File_write_all(fh, buf + (bx + 2 + 1), 1, memtype, MPI_STATUS_IGNORE);
                         ...
                         double *buf)
{
    ...
    /* read data */
    err = MPI_File_read_all(fh, buf + (bx + 2 + 1), 1, memtype, MPI_STATUS_IGNORE);
