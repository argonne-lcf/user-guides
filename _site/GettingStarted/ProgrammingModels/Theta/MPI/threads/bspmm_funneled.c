 ...
 * distribution, and MPI_THREAD_FUNNELED mode.
 *
 ...
 * the submatrix of C that it computes. The DGEMM matrix mutliplication is parallelized
 * using threads.
 */

int is_zero_local(double *local_mat);

void parallel_dgemm(double *local_a, double *local_b, double *local_c);
...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        for (global_j = 0; global_j < blk_num; global_j++) {
            ...
            parallel_dgemm(local_a, local_b, local_c);
...
#pragma omp parallel for private(j,i,k) collapse(2)
    for (j = 0; j < BLK_DIM; j++) {
        for (i = 0; i < BLK_DIM; i++) {
            for (k = 0; k < BLK_DIM; k++)
