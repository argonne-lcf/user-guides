...
Today it is a common practice to use a CI tool for defining pipelines and executing the tasks required to take code from a source stored in a version control system to compiled and packaged artifacts executing in production. Two excellent examples of CI tools are [Jenkins](https://jenkins.io/){:target="_blank"} and [GitLab](https://about.gitlab.com/){:target="_blank"}.
...
Additional information, technical and user documentation, and community support can be found on the [Jenkin's project website](https://jenkins.io/){:target="_blank"}.

###ALCF Jenkins
Log in to the [ALCF Jenkins web portal](https://ci.alcf.anl.gov/jenkins){:target="_blank"} using your ALCF credentials (ALCF username and cryptocard token password).
